...
export class Colonist {
  ...
  update(deltaTime: number, world: Biome[][], resources: { food: number; wood: number; stone: number }) {
    // Update needs
    this.needs.hunger -= deltaTime * 2
    this.needs.rest -= deltaTime

    if (this.needs.hunger <= 0 || this.needs.rest <= 0) {
  ...
  findNewTarget(world: Biome[][]) {
    ...
    const endX = Math.min(world[0].length - 1, Math.floor(this.x) + searchRadius)
    const startY = Math.max(0, Math.floor(this.y) - searchRadius)
    const endY = Math.min(world.length - 1, Math.floor(this.y) + searchRadius)
    ...
    if (bestX !== -1 && bestY !== -1) {
      ...
      this.targetX = Math.floor(Math.random() * world[0].length)
      this.targetY = Math.floor(Math.random() * world.length)
